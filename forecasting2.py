# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 22:41:23 2022
...
#splitting of data
train = df.head(110) # training data
...
#2.holt method
hw = Holt(train["Sales"]).fit(smoothing_level=0.1, smoothing_slope=0.2)
pred_hw = hw.predict(start = test.index[0],end = test.index[-1])
MAPE(pred_hw,test.Sales)
#Holts winter exponential smoothing with additive seasonality and additive trend
hwe_add_add = ExponentialSmoothing(train["Sales"],seasonal="add",trend="add",seasonal_periods=12).fit(smoothing_level=0.1, smoothing_slope=0.2) #add the trend to the model
...
from numpy import sqrt
hw_mul_add = ExponentialSmoothing(train["Sales"],seasonal="mul",trend="add",seasonal_periods=12).fit(smoothing_level=0.1, smoothing_slope=0.2)
pred_hw_mul_add = hw_mul_add.predict(start = test.index[0],end = test.index[-1])
MAPE(pred_hw_mul_add,test.Sales)
rmse_hw_mul_add = sqrt(mean_squared_error(pred_hw_mul_add,test.Sales))
rmse_hw_mul_add
#Final Model by combining train and test
hw_model_add_add = ExponentialSmoothing(interpolated["Sales"],seasonal="add",trend="add",seasonal_periods=10).fit()
